...
#
# Tags look like `v<MRS version>-<release>`:
#
# - if defaults/main.yml points at an MRS version that has never been
#   released, the release counter restarts at 0 (`v0.1.0-0`)
# - otherwise the counter is incremented (`v0.1.0-16`), but only if something
...
# Anchored on `mrs_version:` so that neither the similarly named
# `mrs_container_image_self_build_repo_version:` nor a commented-out line
# can be mistaken for it.
version="$(sed -nE 's|^mrs_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the MRS version from $defaults_path"
	exit 1
fi

# The version values already carry a leading `v` (e.g. `v0.1.0`),
